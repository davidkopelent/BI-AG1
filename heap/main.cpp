...
private:
    ...
    bool hasSons(size_t index) const
    {
        return (index * 2 <= m_H || index * 2 + 1 <= m_H);
    }

public:
    BinaryHeap()
        : ROOT(1), m_H(0), m_Cmp(Comp())
    {
        m_Heap.push_back(T());
    }
    ...
    T extract_min()
    {
        ...
        T min = std::move(m_Heap.at(ROOT));
        std::swap(m_Heap[ROOT], m_Heap[m_H]);
        m_H = m_H - 1;
        m_Heap.pop_back();
        ...
        while (hasSons(v))
        {
            size_t left = 2 * v;
            size_t right = left + 1;
            size_t minChild = left;

            if (right <= m_H && m_Cmp(m_Heap[right], m_Heap[left]))
            {
                minChild = right;
            }

            if (!m_Cmp(m_Heap[minChild], m_Heap[v]))
            {
                break;
            }

            std::swap(m_Heap[v], m_Heap[minChild]);
            v = minChild;
        }
    ...
    void push(T val)
    {
        ...
        while (v != ROOT)
        {
            size_t parent = floor(v / 2);
            if (!m_Cmp(m_Heap[v], m_Heap[parent]))
            {
                return;
            }
            std::swap(m_Heap[v], m_Heap[parent]);
            v = parent;
        }
    }
    ...
    struct TestHelper
    {
        static const T &index_to_value(const BinaryHeap &H, size_t index) { return H.m_Heap[index]; }
        static size_t root_index() { return 1; }
        static size_t parent(size_t index) { return floor(index / 2); }
        static size_t child(size_t parent, size_t ith) { return 2 * parent + ith; }
    };
};
